...
## Changes by me (overview) 
NOTE: Everything should be able to be run the same as described above but for exact recreation, some changes to train.sh might be necessary (no automatic dropout changing is implemented and the python file has to be addionally at the end)

    ./scripts/make_virtualenv.sh
> I made some minor changes for compatibility with my device (I used git bash but there were still some issues so this was easier)
...
(Ultimately I trained with dropout rates of 0.0, 0.2, 0.5, 0.8, 1.0)
    ...
    python line_chart_ppl.py
        Plot training and validation perplexities from log files
